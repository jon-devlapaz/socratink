// Assistant-turn markdown subset. Untrusted model text becomes a typed DOM
// tree (createElement / createTextNode). This is not CommonMark or GFM.

export type MarkdownBlock =
	| { type: 'paragraph'; raw: string; text: string }
	| { type: 'heading'; raw: string; level: 1 | 2 | 3 | 4 | 5 | 6; text: string }
	| { type: 'code'; raw: string; language: string; text: string }
	| { type: 'blockquote'; raw: string; text: string }
	| { type: 'list'; raw: string; ordered: boolean; items: string[] }
	| { type: 'table'; raw: string; header: string[]; rows: string[][] }
	| { type: 'hr'; raw: string };
...
function matchHeading(line: string): Extract<MarkdownBlock, { type: 'heading' }> | undefined {
	...
	return { type: 'heading', raw: line, level, text: (match[3] ?? '').trim() };
}
...
function readFence(
	...
	return {
		block: { type: 'code', raw, language, text: body.join('\n') },
		next: index,
	};
}
...
function readParagraph(
	...
	return {
		block: { type: 'paragraph', raw: body.join('\n'), text: body.join('\n').trim() },
		next: index,
	};
}
...
function parseInline(source: string, flags: InlineFlags = { strong: true, em: true, links: true }): InlineNode[] {
